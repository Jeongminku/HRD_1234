		...
		<form name="frm" action="update"> <!--update로 해야 스위치문으로 update를 찾음(?)-->
			...
			<div class="wrapper">
				<table>
					...
					<tr>
						<th>회원전화</th>
						<td><input type="text" name="phone" class="custphone" value=<%=m.getPhone() %>></td>
					</tr>
					<tr>
						<th>회원주소</th>
						<td><input type="text" name="address" class="custaddress" value=<%=m.getAddress() %>></td>
					</tr>
					<tr>
						<th>가입일자</th>
					...
					<tr>
						...
						<button class="btn" type="button" onclick="location='list'">조 회</button> <!-- 해당 위치로 이동시키는 location -->
						</td>
					</tr>
